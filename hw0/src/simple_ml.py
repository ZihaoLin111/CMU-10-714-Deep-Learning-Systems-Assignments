...
def softmax_loss(Z, y):
    ...
    result = 0.0
    for i in range(n):
        sum_exp = np.sum(np.exp(Z[i]))
        result += np.log(sum_exp) - Z[i, y[i]]
    result /= n
    return result
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, n, batch):
        X_batch = X[i: i+batch]
        y_batch=y[i: i+batch]
        S = np.dot(X_batch, theta)
        Z = np.exp(S)/np.sum(np.exp(S), axis=1, keepdims=True)
        Z[np.arange(len(y_batch)), y_batch] -= 1
        grad = np.dot(X_batch.T, Z) / len(y_batch)
        theta -= lr * grad
